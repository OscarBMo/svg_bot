...
async fn generate_svg_logo<M: CompletionModel>(llm_agent: Agent<M>, item: &InputItem) -> String {
    let svg_generation_prompt = format!(
        r#"Please generate an svg logo based on the following information.
        Provide just svg code, without any additional explanations or markup.
        
        Title: {}
        Description of the desired logo: {}
        "#,
        item.title, item.desired_svg_description
    );
